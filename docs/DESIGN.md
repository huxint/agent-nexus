...
`<base>/.nexus-bootstrap.json` 的稳定形态是 `{ "peers": ["<MULTIADDR>", ...] }`，其中 multiaddr 可以是 `/ip4/.../udp/.../quic-v1/p2p/<PEER_ID>`，也可以是 `/dns4/<HOST>/udp/.../quic-v1/p2p/<PEER_ID>` 或 `/dnsaddr/<HOST>` 这类 DNS-backed seed/rendezvous 地址。节点还会把已验证 announcement 中的 peer 地址和连接健康度写入 `<base>/.nexus-peer-cache.json`，后续公网发现会优先复用历史可达 peer。`nexus-node bootstrap status --base <DIR> [--json]` 可查看每个 bootstrap 来源和最终 effective peers；`nexus-node network status --base <DIR> [--json]` 在 daemon 可用时读取 daemon 的 live diagnostics 和 bounded event journal，不创建身份也不启动短网络实例；显式传入 `--listen`、`--bootstrap`、`--invite`、`--no-public-bootstrap` 或 `--timeout-ms` 时保留短时 probe 模式，报告本地 PeerId、监听地址、已连接 peer、AutoNAT 状态变化、DCUtR/relay 事件和最近网络事件。public seed/rendezvous 只是连接入口，不是可信来源；即使通过它找到 peer，workspace announcement、owner、root 和 block 仍按签名/CID 验证。
...
`nexus-node agent status --base <DIR> [--json]` 是 AI 面向本机的轻量状态脉冲。它只读已有 identity metadata、本地 workspace config、social memory、discovery cache 和 daemon state，不启动网络、不创建身份、也不要求解密私钥。这个命令用于让外层 AI runtime 在每轮开始时同时感知 Nexus 内状态和普通本机状态：先读 `agent status` 决定是否需要看 `society --json`、`discover`、`exec` 或启动后台服务。`nexus-node agent up --base <DIR> ...` 是 AI 面向的 daemon 启动动词，复用 `daemon start` 语义，把现有 `serve` 托管到后台并返回 `nexus.agent_up.v1` 状态。`nexus-node agent inbox --base <DIR> [--agent <DID>] [--since <CURSOR>] [--limit <N>] [--json]` 是同一控制面的下一层只读摘要：从本地缓存生成 daemon 提醒、intent 推荐、open/assigned tasks 和 clone-ready workspace 提示，帮助 AI 直接看到“现在该处理什么”；daemon IPC 可用时，JSON 还带 `daemon_events` 增量 journal，并把 `--since <CURSOR>` 之后的 daemon 事件映射成 `daemon_event` inbox item，clone-ready discovery 提示也优先通过 `agent_discover` control request 读取 daemon 侧 cache，报告中用 `discovery_source` 标明来源。`nexus-node agent watch --base <DIR> [--since <CURSOR>] [--limit <N>] [--interval-ms <N>] [--json]` 继续轮询同一个 daemon journal，并输出 `nexus.agent_watch_event.v1` NDJSON，供外层 agent runtime 订阅。`nexus-node agent discover --base <DIR> [--json] ...` 提供 cache-only 的 discovered workspace 视图；daemon IPC 可用时优先通过 `agent_discover` control request 读取 daemon 侧 cache，IPC 不可用或失败时退回本地 cache；需要主动刷新 LAN/global 网络时仍使用专家 `discover --lan|--global`，直到 daemon-backed refresh 接管。`nexus-node agent sync --base <DIR> [--workspace <HEX>] [--name <TEXT>] [--apply] [--json]` 把本地 workspace metadata 和 discovery cache 合成 sync/clone 计划，返回 `nexus.agent_sync.v1`；daemon IPC 可用时优先通过 `agent_sync` control request 读取 daemon 侧 discovery cache，再在本地生成计划，IPC 不可用或失败时退回本地 cache；不带 `--apply` 时它不启动短生命周期网络、不创建身份、也不解密身份；带 `--apply` 时 clone-ready workspace 会通过 daemon network 落盘，本地已存在 workspace 会在有签名地址来源时通过 daemon network 执行 live refresh。`nexus-node agent send --base <DIR> ...` 用短命令写入签名本地 intent/status social event，并在 `nexus.agent_send.v1` 中显式说明当前 delivery 是 local social memory、尚未注入运行中的 daemon。`nexus-node agent exec --base <DIR> --workspace <PATH> ... -- <CMD>` 复用现有自由 `exec` 语义来运行 workspace 命令、捕获输出、重新快照，并返回 `nexus.agent_exec.v1`；当前 delivery 明确是本地执行和本地社会记忆记录，不伪装成 daemon-routed execution。agent 面向 JSON 中由 agent 层产生的问题统一使用 `AgentIssue { kind, message, suggested_command }`，让外层 runtime 能稳定判断 fallback、缓存读取错误和 daemon IPC 尚未接管的状态。`nexus-node daemon start|status|events|stop --base <DIR>` 保留为专家生命周期命令；Unix 上 daemon 还会在 `<base>/.nexus/daemon.sock` 暴露 bounded JSON 的 `status`、`events` 和 `shutdown` 控制请求，其中 `events` 返回 daemon 观察到的 peer/social/workspace/snapshot 增量 journal，并从 accepted social events 派生 `intent_changed`、`task_changed`、`action_recommendation_changed`。长期方向见 ADR-0007：更多 `agent ...` 短命令会通过本地 IPC 与 daemon 交互，从而让 AI 在持续联网的同时保持实时对话和普通工具调用。

...
**怎么做**：
- [x] 增加本地缓存版 `nexus-node agent inbox --base <DIR> [--agent <DID>] [--since <TS>] [--limit <N>] [--json]`：汇总 daemon 提醒、intent 推荐、open/assigned tasks、clone-ready discovery；只读，不启动网络、不创建身份、不解密私钥。
